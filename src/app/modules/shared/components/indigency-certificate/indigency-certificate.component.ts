...
export class IndigencyCertificateComponent implements OnInit {
  ...
  get isLowIncome(): boolean {
    return (this.certificate?.certificateType || '').toLowerCase().includes('low income');
  }

  get welfareStatusLabel(): string {
    return this.isLowIncome ? 'a low-income resident' : 'indigent';
  }
}
